...
int main(int argc, char **argv) {
    {
        std::vector<int> vec{1, 2, 3, 4, 55};
        ASSERT(vec.size() == 5, "Fill in the correct value.");
        // THINK: `std::vector` 的大小是什么意思？与什么有关？

        // std::vector 内部通常需要存储以下三个指针（或等效信息）：
        // 成员	                作用	                                                大小（64位）
        // pointer to start	    指向第一个元素的指针	                                    8 字节
        // pointer to end	    指向最后一个元素之后的位置（size = end - start）	         8 字节
        // pointer to capacity	指向已分配内存末尾（capacity = capacity_ptr - start）	    8 字节
